...
- [ ] [bug] win screen input + z-order — two issues: (1) player can still move and slash while win screen is showing; (2) debug overlay renders beneath win screen instead of above it. diagnosis: (1) movement dx/dy and trySlash() in `src/main.js` update() not gated on `!gameWon`; (2) `drawDebug()` called before `drawWinScreen()` in render loop. fix: (1) wrap movement input block and trySlash calls in `if (!gameWon)`; (2) move `drawDebug()` call to after `drawWinScreen()` in loop. file: `src/main.js` only. (priority: med)

- [ ] [bug] E re-triggers win screen after debt cleared — symptom: after dismissing win screen, pressing E near payment zone shows win screen again. diagnosis: `src/main.js` line 329 gates E handler on `!gameWon` but not `!isDebtCleared()`; after win dismissed `gameWon` is false, so E fires `payDebt(0)` + `isDebtCleared()` true → `gameWon = true` again. fix: change line 329 condition to `nearZone && !gameWon && !isDebtCleared()`. file: `src/main.js` only. (priority: high)
